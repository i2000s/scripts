...
int main(int argc, char **argv)
{
 ...
 sprintf(runname,"run_dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d_%d.bs"
 ...
 for(i=0;i<nint;i++){
   ...
   sprintf(inname,"dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d.in"
   ...
   fprintf(fin,"\n");
   fprintf(fin," calc full-near-field dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d.dat\n",N,lam,reps,ieps,a,x,y,q0i,q1i,nq);
   ...
   // PBS file
   sprintf(pbsname,"dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d.pbs",N,lam,reps,ieps,a,x,y,q0i,q1i,nq);
   FILE *fpbs;
   fpbs=fopen(pbsname,"w");

   fprintf(fpbs,"#PBS -N dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d\n",N,lam,reps,ieps,a,x,y,q0i,q1i,nq);
   ...
   fprintf(fpbs,"/users/qxd/bem2D/./bem2D dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d.in\n",N,lam,reps,ieps,a,x,y,q0i,q1i,nq);
   fclose(fpbs);

   // BS file
   frun=fopen(runname,"a");
   fprintf(frun,"qsub dipolez_E_N_%d_lam_%g_eps_%g_%g_a_%g_x_%g_y_%g_q_%g_%g_%d.pbs\n",N,lam,reps,ieps,a,x,y,q0i,q1i,nq);
   fclose(frun);
